...
3. **CoA** for APIs (**Optional**)
    ...
    if st.button("Initialize/Run Pipeline"):
        ...
        flat_rlds, current_RLDs = await run_before_interruption(initial_state, thread_info)
...
4. **Proceed to DailyMed Research with SELECTED APIs**
        ...
        if decision == "Retry with dosage forms":
            ...
            api_list_input = st.text_input("API names to update (comma-separated):", key="api_list_input")
            dosage_list_input = st.text_input("New dosage forms (comma-separated, same length as APIs):", key="dosage_list_input")
        if decision == "Retry with API name":
            st.info("Reset brand_name to API name for specified RLDs.")
            retry_api_input = st.text_input("API names to reset brand name (comma-separated):", key="retry_api_input")
        if decision == "Proceed to DailyMed Research with SELECTED APIs":
            ...
            selected_indexes_str = st.text_input("Indexes (e.g., '0,2'):", key="selected_indexes_str")

        if st.button("Resume Pipeline", key="resume_pipeline"):
            human_response = {"feedback_decision": decision}
            if decision == "Retry with API name":
                to_reset = [x.strip() for x in st.session_state.get("retry_api_input", "").split(",") if x.strip()]
                ...
                apis_arr = [x.strip() for x in st.session_state.get("api_list_input", "").split(",") if x.strip()]
                dosage_arr = [x.strip() for x in st.session_state.get("dosage_list_input", "").split(",") if x.strip()]
                mapping = {}
                ...
                human_response["selected_RLDs"] = st.session_state["flat_potential_rlds"]

            elif decision == "Proceed to DailyMed Research with SELECTED APIs":
                selected_indexes = []
                try:
                    selected_indexes = [int(x.strip()) for x in st.session_state.get("selected_indexes_str", "").split(",") if x.strip().isdigit()]
                except Exception:
                    selected_indexes = []
                chosen_rlds = [st.session_state["flat_potential_rlds"][i] for i in selected_indexes if i < len(st.session_state["flat_potential_rlds"])]
            ...
            final_decisions = ["Proceed to DailyMed Research AS IS", "Proceed to DailyMed Research with SELECTED APIs"]
            if human_response["feedback_decision"] not in final_decisions:
                ...
                loop = st.session_state["loop"]
                flat_rlds, current_RLDs = await run_after_interruption(human_response, thread_info)
